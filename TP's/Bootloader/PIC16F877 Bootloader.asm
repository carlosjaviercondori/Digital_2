...
__CONFIG _FOSC_HS & _WDTE_OFF & _PWRTE_ON & _CP_OFF & _BOREN_ON & _LVP_OFF & _CPD_OFF & _WRT_OFF
; *****************************************************************************
...
StartWriteB3:
    ...
    END
; Comentar o eliminar líneas como estas si existen:
; banksel ANSEL
; clrf    ANSEL           ; PIC16F877A no tiene este registro
; clrf    ANSELH          ; PIC16F877A no tiene este registro

; Para configurar pines digitales en PIC16F877A, usar ADCON1
    banksel ADCON1
    movlw   0x06            ; Todos los pines digitales (no analógicos)
    movwf   ADCON1

; Configuración USART para PIC16F877A
; Solo usar TXSTA, RCSTA y SPBRG (no BAUDCTL)
    
    banksel TXSTA
    movlw   (1<<TXEN)       ; Habilitar transmisión
    movwf   TXSTA
    
    banksel RCSTA
    movlw   (1<<SPEN)|(1<<CREN) ; Habilitar puerto serie y recepción
    movwf   RCSTA
    
    banksel SPBRG
    movlw   .25             ; Baud rate para 20MHz (ejemplo: 19200 bps)
    movwf   SPBRG

; Verificar que las direcciones sean correctas para PIC16F877A
; Ejemplo de ubicación del bootloader al final de la memoria:
BOOTLOADER_START    EQU    0x1F00    ; Inicio del bootloader
USER_RESET_VECTOR   EQU    0x0000    ; Vector de reset del usuario
USER_INT_VECTOR     EQU    0x0004    ; Vector de interrupción del usuario
