...
INICIO:
    ...
    ; ---------- UART TX setup ----------
    BANKSEL TRISC
    BSF     STATUS, RP0         ; seleccionar banco 1 si hace falta
    ; asegurarse RC6 como salida (TX)
    BANKSEL TRISC
    BCF     TRISC, 6            ; RC6 (TX) como salida
    ; Configurar TXSTA/RCSTA/SPBRG (BRGH=1, TXEN=1, SPEN=1)
    BANKSEL TXSTA
    MOVLW   b'00100100'         ; BRGH=1, TXEN=1 (TXSTA = 0x24)
    MOVWF   TXSTA
    BANKSEL RCSTA
    MOVLW   b'10010000'         ; SPEN=1, CREN=0 (solo TX)
    MOVWF   RCSTA
    BANKSEL SPBRG
    MOVLW   D'51'               ; SPBRG para 19200 bps con Fosc=4MHz y BRGH=1
    MOVWF   SPBRG
    ; -----------------------------------
...
SEND_ADRESH:
    ; W_temp usado por convención: mov ADC8 -> W y enviar
    BANKSEL ADC8
    MOVF    ADC8, W
    ; esperar TXIF = 1
WAIT_TX_READY:
